...
Our last step in EDA was to look at a 
...
data_copy <- subset(data_copy, select = -c(Hospital_code,case_id,Hospital_region_code, patientid,City_Code_Patient,City_Code_Hospital,Ward_Facility_Code,Visitors.with.Patient))
...
get_accuracy_rate(AllResults, AllLabels)
```
Model 2 yielded a much greater accuracy, 91.38%. However, the confusion matrix shows the model had a very low true positive rate, 1.80%.
...
Model 1 yielded very low accuracy, 46.03%, and was very limited in its ability to predict lengths of stay greater than 60 days. 
   
For model 2, we used the same independent variables as model 1 but we converted 'Stay' to a binary variable denoting length of stay was 61 days or more. The model yielded much higher accuracy than model 1, 91.37%, but this was mostly due to correctly predicting true negatives. 

To solve the low true positives issue, we included "Hospital_code","Hospital_region_code", and "Visitors.with.Patient". This yielded a significantly higher true positive rate, 34.61% (from 1.80%) , and a mildly improved accuracy. 

Model 4 used the same predictors as model 3, but tried to predict "Stay". It saw improvement over model 1, but still only had 50.37% accuracy. 
